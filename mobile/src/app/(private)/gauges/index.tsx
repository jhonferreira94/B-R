...
import type { Gauge } from '@/features/gauges/schemas/gauges.schema';
...
export default function GaugesListScreen() {
  const { session } = useAuth();
  const [search, setSearch] = useState('');
  const debouncedSearch = useDebounce(search, 400);

  const canCreate = hasPermission(session?.user, [KnownClaims.create_gauges]);
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useGaugesInfinite({ search: debouncedSearch || undefined, pageSize: 20 });

  const items: Gauge[] = useMemo(
  ...
  const headerRight = canCreate ? (
    <Button
      ...
      onPress={() => router.push('/gauges/create')}
    >
  ...
  return (
    <ScreenListLayout<Gauge>
      ...
      renderItem={({ item }) => (
        <TouchableOpacity
          activeOpacity={0.7}
          onPress={() => router.push(`/gauges/${item.id}`)}
        ...
        >
          <VStack>
            <Text className="font-bold text-typography-950 text-lg">{item.awg}</Text>
        ...
        isLoading ? (
          ...
          <VStack className="items-center gap-3 mt-10 px-6">
            <Text className="text-typography-500 text-center">
              Não foi possível carregar as bitolas.
            </Text>
          ...
          <VStack className="items-center gap-3 mt-10 px-6">
            <Text className="text-typography-500 text-center">
              {debouncedSearch
                ? 'Nenhuma bitola encontrada para essa busca.'
                : 'Nenhuma bitola cadastrada ainda.'}
            </Text>
          </VStack>
        )
      }
