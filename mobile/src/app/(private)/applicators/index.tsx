...
import type { Applicator } from '@/features/applicators/schemas/applicators.schema';
...
export default function ApplicatorsListScreen() {
  const { session } = useAuth();
  const [search, setSearch] = useState('');
  const debouncedSearch = useDebounce(search, 400);

  const canCreate = hasPermission(session?.user, [KnownClaims.create_applicators]);
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useApplicatorsInfinite({ search: debouncedSearch || undefined, pageSize: 20 });

  const items: Applicator[] = useMemo(
  ...
  const headerRight = canCreate ? (
    <Button
      ...
      onPress={() => router.push('/applicators/create')}
    >
  ...
  return (
    <ScreenListLayout<Applicator>
      ...
      renderItem={({ item }) => (
        <TouchableOpacity
          activeOpacity={0.7}
          onPress={() => router.push(`/applicators/${item.id}`)}
          className="rounded-lg border border-outline-200 bg-background-0 p-4 shadow-sm shadow-background-900/5"
        >
          <HStack className="items-center justify-between mb-2">
            <Text className="font-bold text-typography-950 text-base tracking-widest">{item.code}</Text>
            <ChevronRight size={18} color="#94A3B8" />
          </HStack>
          
          <Text size="sm" className="text-typography-800 mb-3 font-semibold">
            {item.name}
          </Text>

          <HStack className="items-center justify-between">
            <Text size="xs" className="text-typography-500 font-medium">
              Ciclos de Manutenção: <Text size="xs" className="text-typography-900 font-semibold">{item.maintenanceCycles}</Text>
            </Text>
          </HStack>
        </TouchableOpacity>
      )}
        ...
        isLoading ? (
          ...
          <VStack className="items-center gap-3 mt-10 px-6">
            <Text className="text-typography-500 text-center">
              Não foi possível carregar os aplicadores.
            </Text>
          ...
          <VStack className="items-center gap-3 mt-10 px-6">
            <Text className="text-typography-500 text-center">
              {debouncedSearch
                ? 'Nenhum aplicador encontrado para essa busca.'
